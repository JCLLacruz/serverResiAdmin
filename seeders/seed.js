...
const createSessions = async (activities, residents) => {
	...
	const endDate = new Date('2024-08-31'); // Fin de agosto
	const maxSessions = 60; // Máximo de 60 sesiones en total
	const minSessions = 20; // Mínimo de 20 sesiones en total

	let totalSessions = 0;

	// Crear sesiones en fechas aleatorias entre julio y agosto
	while (totalSessions < maxSessions || totalSessions < minSessions) {
		const randomDate = getRandomDate(startDate, endDate);
		const activity = activities[Math.floor(Math.random() * activities.length)];

		// Seleccionar un número aleatorio de residentes entre 8 y 12
		const numResidents = Math.floor(Math.random() * 5) + 8; // Aleatorio entre 8 y 12

		let sessionResidents = [];
		let selectedGroup = null; // Para almacenar el grupo seleccionado

		while (sessionResidents.length < numResidents) {
			const randomResident = residents[Math.floor(Math.random() * residents.length)];
			if (!sessionResidents.includes(randomResident._id)) {
				// Comprobar si el grupo ha sido seleccionado
				if (!selectedGroup) {
					selectedGroup = randomResident.group;
				} else {
					// Verificar que todos los residentes seleccionados pertenecen al mismo grupo
					if (randomResident.group.identificator !== selectedGroup.identificator || randomResident.group.subdivision !== selectedGroup.subdivision) {
						continue; // Si no coincide, saltar a la siguiente iteración
					}
				}
		...
		// Crear la sesión con la fecha y el grupo
		const session = new Session({
			activityId: activity._id,
			observations: faker.lorem.sentence(),
			residentIds: sessionResidents,
			createdAt: randomDate,
			group: selectedGroup, // Incluir el grupo en la sesión
		});
		...
		await Promise.all(
			sessionResidents.map(async (residentId) => {
	...
	for (const resident of residents) {
		...
		for (const record of attendanceRecords) {
			const sessionOnDate = sessions.find((session) => {
				return (
					session.createdAt.toISOString().split('T')[0] === record.date.toISOString().split('T')[0] && session.residentIds.includes(resident._id)
				);
			});
		...
		await Resident.updateOne({ _id: resident._id }, { $set: { attendance: attendanceRecords } });
	}
